...
var methods = {
  ...
  getSelectedIds: getSelectedIds, deselectAll: deselectAll, getDomNode: getDomNode,
...
TemplateClass.rendered = function() {
  ...
  function editItem(args) {
    var id = args.id || getSelectedIds(tableId);
  ...
  function deleteItem() {
    if (confirm('Delete item?')) {
      settings.onDelete ? settings.onDelete(createHandlerContext()) :
          collection.remove(getSelectedIds(tableId));
    }
  }
};
...
TemplateClass.events({
  'click table.selectable tbody tr': function(e, template) {
    ...
    if (_.indexOf(selectedIds, id) >= 0) {
      newSelectedIds = _.without(selectedIds, id);
    } else {
      newSelectedIds = _.union(selectedIds, [id]);
    }
    setSelectedIds(tableId, newSelectedIds);
  },
