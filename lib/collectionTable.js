...
function setSelectedIds(tableId, ids) {
  ...
  removeSelection(tableId, toDeselectIds);
  addSelection(tableId, toSelectIds);
}
...
function deselectAll(tableId) {
  var selectedIds = getSelectedIds(tableId);
  removeSelection(tableId, selectedIds);
  return selectedIds;
}

function toggleSelection(tableId, ids) {
  var selectedIds = getSelectedIds(tableId);
...
function addSelection(tableId, ids) {
  var selectedIds = getSelectedIds(tableId);
  var toSelectIds = _.difference(ids, selectedIds);
  ...
  setReactiveVar(tableId, selectedField, newSelectedIds);
  $(getDomNode(template)).trigger(selectEventName, toSelectIds);
}

function removeSelection(tableId, ids) {
  var selectedIds = getSelectedIds(tableId);
  var toDeselectIds = _.intersection(selectedIds, ids);
  ...
  setReactiveVar(tableId, selectedField, newSelectedIds);
  $(getDomNode(template)).trigger(deselectEventName, toDeselectIds);
}
