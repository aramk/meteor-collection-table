...
var selectionChangeEventName = 'selection/change';
...
function getRow(id, template) {
  var row = _.find(template.$('tbody tr'), function(tr) {
    var data = Blaze.getData(tr);
    return data._id === id;
  });
  return $(row);
}
...
function addSelection(domNode, ids) {
  ...
  var settings = getSettings(domNode);
  var selectedIds = getSelectedIds(domNode);
  var toSelectIds = _.difference(ids, selectedIds);
  ...
  $(domNode).trigger(selectionChangeEventName, {select: toSelectIds});
}
...
function removeSelection(domNode, ids) {
  ...
  $(domNode).trigger(selectionChangeEventName, {deselect: toDeselectIds});
}
...
function getSettings(domNode) {
  return getTemplate(domNode).data.settings || {};
}
...
function configureSettings(template) {
  ...
  if (!collection) {
    ...
    if (collectionName) {
      ...
      template.createRoute = data.createRoute || collectionId + 'Create';
...
TemplateClass.rendered = function() {
  ...
  var $table = $(this.$('.reactive-table')).addClass('ui selectable table segment');
  var $collectionTable = this.$('.collection-table');
...
TemplateClass.events({
  ...
  'change [type="checkbox"]': function(e, template) {
    var settings = template.data.settings;
    if (settings.checkbox) {
      var $checkbox = $(e.currentTarget);
      var data = Blaze.getData($checkbox.parent().parent()[0]);
      var $table = $(getDomNode(template));
      $table.trigger('check', {
        $table: $table,
        $checkbox: $checkbox,
        data: data,
  ...
  'click [type="checkbox"]': function(e, template) {
...
TemplateClass.helpers({
  ...
  tableSettings: function() {
    ...
    // instances.
    var settings = Setter.merge(Setter.clone(data.settings), {
    ...
    if (checkbox) {
      var checkboxField = {
        ...
        fn: function(value, object) {
          var getValue = checkbox.getValue;
          if (getValue) {
